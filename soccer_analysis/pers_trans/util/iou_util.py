...
class IouUtil:
    ...
    def homography_warp(h, image, dst_size, background_color):
        ...
        """
        assert h.shape == (3, 3)
        im_dst = cv.warpPerspective(image,
                                    h,
                                    dst_size,
    ...
    @staticmethod
    def template_to_image_homography_uot(camera,
                                         template_h=74,
                                         template_w=115):
        """
        ...
        m1 = np.asarray([[1, 0, 0], [0, -1, template_h], [0, 0, 1]])
        # scale
        m2 = np.asarray([[yard2meter, 0, 0], [0, yard2meter, 0], [0, 0, 1]])
    ...
    def iou_on_template_uot(gt_h,
        ...
        gt_mask = IouUtil.homography_warp(np.linalg.inv(gt_h), im,
                                          (template_w, template_h), (0))
        pred_mask = IouUtil.homography_warp(np.linalg.inv(pred_h), im,
                                            (template_w, template_h), (0))
...
def ut_homography_warp():
    camera_data = np.asarray([
        640, 360, 3081.976880, 1.746393, -0.321347, 0.266827, 52.816224,
        -54.753716, 19.960425
    ])
...
def ut_generate_grassland_mask():
    ...
    data = sio.loadmat(
        r'C:\Users\mostafa\Desktop\test\SCCvSD-master/data/UoT_soccer/train_val.mat'
    )
