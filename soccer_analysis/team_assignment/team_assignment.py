...
file_dict = {
    'frej-bp.txt': ['White-Yellow.png', 'Yellow-White.png'],
    'IF Brommapojkarna - Örebro Syrianska IF.txt':
    ['Yellow-RedBlack.png', 'RedBlack-Yellow.png'],
    'IF Sylvia - Team TG FF.txt': ['BlackWhite-Red.png', 'Red-BlackWhite.png'],
    'IFK Berga - IF Sylvia.txt': ['Orange-White.png', 'White-Orange.png'],
    'IFK Värnamo - FC Linköping City.txt':
    ['White-Blue.png', 'Blue-White.png'],
    'Nyköpings BIS - IFK Haninge.txt': ['Black-Red.png', 'Red-Black.png'],
    'Qviding FIF - Landskrona BoIS.txt':
    ['Dark-BlackWhite.png', 'BlackWhite-Dark.png'],
    ...
    'Tvååkers IF - Landskrona BoIS.txt':
    ['BlackWhite-Red2.png', 'Red-BlackWhite2.png'],
...
def detect_color(img, colors, debug=False):
    ...
    assigned_color = (int(assigned_color[0]), int(assigned_color[1]),
                      int(assigned_color[2]))
...
def team_assignment(crop, infoFile, debug=False):
    ...
    with open(infoFile, 'r') as f:
        data = f.read()
        two_colors = re.findall(r'team, (.*?) [J,j]erseys[)]', data)
        colors = []
        for color in two_colors:
            if color.find('/') != -1:
                ...
                newcolor = ((All_colors[newcolors[0]][0] +
                             All_colors[newcolors[1]][0]) / 2,
                            (All_colors[newcolors[0]][1] +
                             All_colors[newcolors[1]][1]) / 2,
                            (All_colors[newcolors[0]][2] +
    ...
    color, colorName = detect_color(crop, colors, debug=debug)
    return colorName
    ...
    for (txt_file, image_file) in file_dict.items():
        ...
        for image_path in image_file:
            ...
            if colorName in results:
                ...
                logger.info(f'processing file: {txt_file} and {image_path},' +
