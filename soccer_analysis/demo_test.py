...
from team_assignment.team_assignment import team_assignment
...
class Analysis(object):
    ...
    def __init__(self,
        ...
        self.save_dir = increment_path(Path(project) / exp_name,
                                       exist_ok=exist_ok)  # increment run
        (self.save_dir / 'tracks' if save_txt else self.save_dir).mkdir(
            parents=True, exist_ok=True)  # make dir

        # Load model
        if eval:
            self.device = torch.device(int(device))
        else:
    ...
    def test_analysis_image(self, source='0', show_team='', imgsz=(640, 640)):
        ...
        webcam = source.isnumeric() or source.endswith('.txt') or (is_url and
                                                                   not is_file)
        ...
        # Run tracking
        self.model.warmup(imgsz=(1 if self.pt else nr_sources, 3,
                                 *imgsz))  # warmup
        ...
        for frame_idx, (path, im, im0s, vid_cap, s) in enumerate(dataset):
            ...
            for i, det in enumerate(pred):  # detections per image
                ...
                if det is not None and len(det):
                    ...
                    if len(outputs[i]) > 0:
                        for j, (output,
                                conf) in enumerate(zip(outputs[i], confs)):
